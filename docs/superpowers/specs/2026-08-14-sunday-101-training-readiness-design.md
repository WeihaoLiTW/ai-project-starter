...
**Windows 學員另需**(若有):`.msix` 安裝、系統管理員權限、Virtual Machine
Platform;工作資料夾必須在 `C:\Users\<使用者>\` 底下。這些是 kit 既有的 Cowork
安裝陷阱,對 101 會靜默失敗,必須進 pre-work 檢查。**目前 Windows 學員比例未知
(待確認)。**
...
是**首頁上一個可見字串**,好讓 staging/prod 的差異在瀏覽器裡直接看得見。
...
三個概念的 101 版講法(不看 code 就懂),每個釘到 demo 的一個時刻:
