...
| staging／prod 網址回不回 200 | 這兩個網址（`*.zeabur.app` 的子網域）是安裝時自己取的名字，這個專案目前沒有任何地方會記下來，我也找不到能穩定列出它的 CLI 指令——先試著用已驗證的 Zeabur 路徑打開服務頁面找網域，找不到就直接問使用者這兩個網址，他自己天天在用，問他最快。查到網域後不要直接 curl 網域本身：這個專案只在 `core/urls.py` 註冊了 `health/` 這一條路徑（見該檔案），根目錄 `/` 沒有對應到任何 view，健康的服務打根目錄一樣是 404；prod 又開了 `SECURE_SSL_REDIRECT`（見 `project/settings/prod.py`），不帶 `https://` 的網址預設會用 http 連，先被轉址成 301。兩者疊起來，健康的服務打錯網址一樣會回紅燈。要跑的是完整網址 `https://<網域>/health/`，對每個環境各跑一次 `curl -s -o /dev/null -w '%{http_code}' https://<網域>/health/`，把回傳碼寫進 `facts["endpoints"]["staging"]` / `["prod"]` |
