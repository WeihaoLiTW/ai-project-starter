...
```python
# tests/test_health_check.py（歷史抽驗那三個）
import sys
import types
...
```python
# tests/test_health_check.py（框架那幾個）
import sys
import types
...
技術決策底下藏著業務取捨的，翻譯成業務問題來問。翻譯靠一個固定的四步驟，
不是靠硬記例子：

1. 這個決定會改變什麼是他**看得到**的？系統實際做什麼、資料留不留得住、
   誰能做什麼、要花多少錢、東西壞掉的時候會發生什麼事——挑得出看得到的
   才算。
2. 沒有一項看得到的東西會變，就沒有業務取捨。自己決定，寫一則 ADR，
   然後告訴他你決定了什麼，不要問。禁問的清單在
   [forbidden-questions.md](../../behavior/forbidden-questions.md)。
3. 有東西會變，業務問題就是：這些看得到的結果裡，他想要哪一種？問結果，
   不要問產生結果的機制。
4. 檢查答案夠不夠用：這個答案能不能讓你直接做技術決定，不用猜？不夠，
   針對同一個結果再追問一次——不要退回去問技術問題。

**套用在下面表格第一列（soft delete 還是 hard delete）**：

1. 看得到的東西：員工離職之後，他填過的班表還在不在系統裡。
2. 有東西會變——刪了紀錄消失，不刪紀錄還在，這是業務取捨，要問。
3. 業務問題：員工離職之後，他填過的班表要保留還是消失？
4. 答案「保留」夠不夠做技術決定？夠——保留就是 soft delete，不用再問。

下面這張表是套這個步驟問出來的結果，當範例參考，不是規則本身：
...
例如他答「應該不會有很多人用」，這還是含糊。不要追著同一句話問「所以大概
是幾個人？」——換句話重複，他答不出來的話還是答不出來。換個講法問：
「你們公司現在總共幾個人？這個系統只有內部同事用，還是也會開放給客戶或
廠商？」從「量」換成「範圍」，他答得出來的機率更高。
